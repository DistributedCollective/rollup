...
task('accounts', 'Prints the list of accounts', async (taskArgs, bre) => {
  ...
  for (const account of accounts) {
    console.warn(await account.getAddress())
  }
})
...
task('create-chain', 'Creates a rollup chain')
  ...
  .setAction(async (taskArguments, hre) => {
    console.warn("init seq")
    
    const machineHash = fs.readFileSync('../MACHINEHASH').toString()
    console.warn(`Creating chain for machine with hash ${machineHash} for sequencer ${taskArguments.sequencer}`)
    ...
    console.log("rollup Creator addr", rollupCreatorDep.address);
...
const config = {
  ...
  networks: {
    ...
    rsktestnet: {
     // url: 'https://rsktestnet.vrun.vn/rpc',
      url: 'http://3.141.186.36:4444',
      accounts: [PrivateKey],
      network_id: 31,
      confirmations: 3,
      gas: 6800000,
